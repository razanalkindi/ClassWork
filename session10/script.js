    ...
    for(i=0; i<arrProdect.length; i++)
    {
        dataTable +=`
            <tr>
                ...
                <td><button id="updete" onclik="updete(${i})">Updete</button></td>
            </tr>
        `
...
let y;
function updete(indexedElement){
    y = indexedElement;
    prodectName.value=arrProdect[indexedElement].pname;
    prodectPrice.value=arrProdect[indexedElement].pprice;
    prodectCatogory.value=arrProdect[indexedElement].pcatogory;
    prodectDescription.value=arrProdect[indexedElement].pdescription;
    addBtn.classList.toggle("show");
    saveBtn.classList.toggle("show");
}

saveBtn.addEventListener('click',()=>{
    arrProdect[y].pname=prodectName.value
    arrProdect[y].pprice=prodectPrice.value
    arrProdect[y].pcatogory=prodectCatogory.value
    arrProdect[y].pdescription=prodectDescription.value
    localStorage.getItem('prodect' ,JSON.stringify(arrProdect))
    addBtn.classList.toggle("show");
    saveBtn.classList.toggle("show");
    loopData();
    clearData();
